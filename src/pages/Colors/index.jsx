...
// eslint-disable-next-line import/no-unresolved, import/extensions
import { addColor, deleteAllColor, deleteColor } from "@/store/colors-actions";
import ColorModal from "@/components/ColorModal";
import DeleteModal from "@/components/DeleteModal";
import { create } from "../../../services/tonner";

import { create as createManufatcurer } from "../../../services/manufactuer";
...
export default function Colors() {
  ...
  const { colors, searching, filteredColors } = useSelector(
    (state) => state.colors
  );
  ...
  const transformColors = useCallback(
    (col) =>
      col.map((color) => ({
        ...color,
        colorShade: <ImageClipboardComponent color={color} />,
  ...
  const filteredTransformedColors = useMemo(
    () => transformColors(filteredColors),
    [filteredColors, manufacturersOptions]
  );
  ...
  function extractColorInfo(fileContent) {
    ...
    colorInfo.yearFrom =
      matchYearFrom && matchYearFrom[1] ? matchYearFrom[1] : "";
    ...
    colorInfo.manufacturerName =
      matchManufacturerName && matchManufacturerName[1]
        ? matchManufacturerName[1]
        : "";

    const matchColorName = fileContent.match(/(Color|Colour) name\s+:\s+(.+)/i);
    colorInfo.colorName =
      matchColorName && matchColorName[2] ? matchColorName[2] : "";

    const matchColorCode = fileContent.match(/Colou?r code\s+:\s+(\w+)/);
    colorInfo.colorCode =
      matchColorCode && matchColorCode[1] ? matchColorCode[1] : "";

    const matchVariant = fileContent.match(
      /Variant\s+:\s*(.*?)\s+(?=Colou?r name)/
    );
    colorInfo.variant =
      matchVariant && matchVariant[1] ? matchVariant[1].trim() : "";

    const matchMutationDate = fileContent.match(/Mutation date\s*:\s*(.*)/);
    colorInfo.modelVersion =
      matchMutationDate && matchMutationDate[1]
        ? matchMutationDate[1].trim()
        : "";
    const matchQuality = fileContent.match(/Quality\s+:\s+([\w\s-]+)/);
    const qualityValue =
      matchQuality && matchQuality[1] ? matchQuality[1].trim() : "";
  ...
  const getManufacturer = async (manufacturerName) => {
    ...
    const { isAlreadyExist, ...rest } = await createManufatcurer(
      manufacturerName
    );
    if (!isAlreadyExist) {
  ...
  function areArraysOfObjectsEqual(arr1, arr2) {
    ...
    for (let i = 0; i < sortedArr1.length; i++) {
      ...
      if (obj1.abs === obj2.abs && obj1.tonner === obj2.tonner) {
  ...
  const alreadyExist = (body) => {
    const nonCombineColors = colors.filter(
      (color) => color.isCombineColor !== true
    );
  ...
  const uploadFiles = async (color, index, totalFiles) => {
    ...
    try {
      ...
      const object = {
        ...
        colorType: manufacturerIdcolorType,
      };
  ...
  const onExport = (options) => {
    const tempHeader = [
      ...
      { label: "Tonner 1", key: "tonner1" },
      { label: "Tonner 2", key: "tonner2" },
      { label: "Tonner 3", key: "tonner3" },
      { label: "Tonner 4", key: "tonner4" },
      { label: "Tonner 5", key: "tonner5" },
      { label: "Tonner 6", key: "tonner6" },
      { label: "Tonner 7", key: "tonner7" },
      { label: "Tonner 8", key: "tonner8" },
      { label: "Tonner 9", key: "tonner9" },
      { label: "Tonner 10", key: "tonner10" },
      { label: "Tonner 11", key: "tonner11" },
      { label: "Tonner 12", key: "tonner12" },
      { label: "Tonner 13", key: "tonner13" },
      { label: "Tonner 14", key: "tonner14" },

      { label: "Abs 1", key: "abs1" },
      { label: "Abs 2", key: "abs2" },
      { label: "Abs 3", key: "abs3" },
      { label: "Abs 4", key: "abs4" },
      { label: "Abs 5", key: "abs5" },
      { label: "Abs 6", key: "abs6" },
      { label: "Abs 7", key: "abs7" },
      { label: "Abs 8", key: "abs8" },
      { label: "Abs 9", key: "abs9" },
      { label: "Abs 10", key: "abs10" },
      { label: "Abs 11", key: "abs11" },
      { label: "Abs 12", key: "abs12" },
      { label: "Abs 13", key: "abs13" },
      { label: "Abs 14", key: "abs14" },
    ];
    ...
    data.forEach((element) => {
      ...
      const obj = { ...rest, ...tonnerDummy ,...absDummy};
  ...
  const handleExcelFileUpload = async (event) => {
    ...
    readXlsxFile(file)
      .then(async (rows) => {
        ...
        console.log("tonnerIndexes",tonnerIndexes)
        console.log("absIndexes",absIndexes)
